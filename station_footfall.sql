...
WITH Stations_Peak_Day AS (
    SELECT 
        Day_Of_Week, 
        Station_Name, 
        ROUND(AVG(Total_Tap_Count),0) AS Station_Footfall,
        ROW_NUMBER() OVER (PARTITION BY Day_Of_Week ORDER BY ROUND(AVG(Total_Tap_Count),0) DESC) AS rn  
    FROM Footfall f
    INNER JOIN Stations s ON f.StationID = s.StationID
    WHERE Travel_Date LIKE '2024%' 
...
-- average footfall per day in 2024? 

WITH Total_Daily_Footfall AS 
	( 
SELECT SUM(Total_Tap_Count) AS Footfall_Per_Day 
	FROM Footfall 
		WHERE Travel_Date LIKE '2024%' 
        GROUP BY Travel_Date
)
SELECT AVG(Footfall_Per_Day) AS AvgFootfall_Per_Day 
	FROM Total_Daily_Footfall; 
...
WITH Station_Footfall AS (
	SELECT Station_Name, Zone, ROUND(AVG(Total_Tap_Count),0) AS Daily_Station_Footfall 
		FROM Stations s
...
Zone_Averages AS (
	SELECT Zone, ROUND(AVG(Total_Tap_Count),0) AS Daily_Zone_Footfall 
		FROM Stations s 
			JOIN Footfall f ON s.StationID = f.StationID
				WHERE Travel_Date LIKE '2024%' 
				GROUP BY Zone 
	) 
SELECT Station_Name, sf.Zone, Daily_Station_Footfall 
